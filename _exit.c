...
int c_atoi(char *s)
{
	...
	while (s[i] != '\0')
	{
		if (s[i] >= '0' && s[i] <= '9') 	/* calc number */
			numb = numb * 10 + (s[i] - '0');
		if (s[i] > '9' || s[i] < '0') /* acct for non-numbers */
			return (-1);
		i++;
	}
	return (numb);
 ...
 * @environment: bring in environmental variable to free at error
 * @number: bring in nth user commnd line input to print in error message*
...
int __exit(char **str, list_t *env, int numb, char **comd)
{
	...
	if (string[1] != NULL) /* if no value given after exit, return 0 */
		e_value = c_atoi(str[1]);

	if (e_value == -1) /* if value given after exit is invalid, perror */
	{
		illegal_numb(str[1], numb, environment); /* print error msg */
		free_double_ptr(string);
		return (2);
	}
	free_double_ptr(string); /* free user input before exiting program */
