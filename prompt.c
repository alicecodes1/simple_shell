...
/**
 * ctrl_c - ignore Control-C input & printing prompt to execution outline
 * @n: takes in intiga from signal
 */
void ctrl_c(int n)
{
	(void)n;
 ...
 * @num: take in nth user cmd  typed to write error message
...
int built_in(char **token, list_t *env, int num, char **command)
{
	...
	/* if user types "exit", free cmd tokens, & exit */
	if (_strcmp(token[0], "exit") == 0)
	{
		i = __exit(token, env, num, command);
	}
	/* if user types "env", print, free cmd tokens, & reprompt */
...
/**
 * ignore_space - return str without spaces in the front
 * @str: str
 * Return: new str
 */
...
/**
 * prompt - repeatedly prompts user & executes user's cmds if applicable
...
int prompt(char **en)
{
	...
	/* keep on repeating till user exits shell */
	return (exit_stat);
}
